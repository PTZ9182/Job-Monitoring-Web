...
<body>
            ...
            <div class="bg-Neutral/01 h-16 flex flex-row justify-between items-center lg:h-20">
                ...
                <div class="mx-5">
                    <div class="flex flex-row">
                        ...
                        <p class="font-medium text-base text-Neutral/14 ml-1 uppercase">{{ Auth::user()->name }}</p>
                    </div>
                </div>
                    ...
                    <div class="bg-Neutral/01 flex flex-row w-full justify-between px-5 py-5 rounded-lg drop-shadow-lg shadow-inner lg:w-1/2 lg:mr-10">
                        ...
                        <div class="flex flex-row items-center">
                            <svg width="30" height="34" viewBox="0 0 37 41" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.7222 20.89H24.3333M10.7222 28.69H19.2389M26.2778 4.939C32.7528 5.29 36 7.6885 36 16.6V28.3C36 36.1 34.0556 40 24.3333 40H12.6667C2.94445 40 1 36.1 1 28.3V16.6C1 7.708 4.24722 5.29 10.7222 4.939M14.6111 8.8H22.3889C26.2778 8.8 26.2778 6.85 26.2778 4.9C26.2778 1 24.3333 1 22.3889 1H14.6111C12.6667 1 10.7222 1 10.7222 4.9C10.7222 8.8 12.6667 8.8 14.6111 8.8Z" stroke="#292D32" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                            <div class="flex flex-col items-center">
                                <p class="font-medium text-lg mx-5 lg:font-medium lg:text-base">Jumlah Divisi</p>
                                <p id="jumlah-Divisi" class="font-medium text-lg lg:font-medium lg:text-base">{{ $count }}</p>
                            </div>
                        </div>
                ...
                <div class="p-5 flex flex-col">
                    ...
                    <table class="text-center">
                        ...
                        <tr class="border-2">
                            ...
                            <td class="font-normal text-sm py-3 ">{{ $dataId->divisi }}</td>
                            <td class="font-normal text-sm py-3 flex flex-row items-center justify-center"> 
                                <button onclick="passDataEdit({{ $dataId->id }}, '{{ $dataId->divisi }}')" type="button" class="items-center mx-2 border px-4 py-1 rounded hover:bg-Neutral/11 lg:mx-5" >      
                                ...
                                <button onclick="passDataDelete({{ $dataId->id }})" type="button" class="items-center mx-2 border px-4 py-1 rounded hover:bg-Neutral/11 lg:mx-5">      
                                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div id="tambahModal" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen bg-gray-500 bg-opacity-50">
            <div class="bg-white w-1/2 p-10 rounded shadow-md">
                ...
                <h2 class="text-center text-base lg:text-2xl font-bold mb-4">Tambah Divisi</h2>
    ...
    <div id="editModal" class="fixed z-10 inset-0 overflow-y-auto hidden">
        <div class="flex items-center justify-center min-h-screen bg-gray-500 bg-opacity-50">
            <div class="bg-white w-1/2 p-10 rounded shadow-md">
                ...
                <h2 class="text-center text-base lg:text-2xl font-bold mb-4">Edit Divisi</h2>
        ...
        <div class="flex items-center justify-center min-h-screen bg-gray-500 bg-opacity-50">
            <div class="bg-white w-auto p-10 rounded shadow-md">
                <div class="flex flex-col p-4 md:p-5 text-center items-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-neutral-800">Hapus Divisi ?</h3>
    ...
    <script>
        ...
        // Modal Tambah Divisi
        const tambahDivisibutton = document.getElementById('tambahDivisibutton');
        const tutupTambahModal = document.getElementById('tutupTambahModal');
        const tambahModal = document.getElementById('tambahModal');
    
        tambahDivisibutton.addEventListener('click', () => {
            tambahModal.classList.remove('hidden');
        });
    
        tutupTambahModal.addEventListener('click', () => {
            tambahModal.classList.add('hidden');
        });

        // Modal Edit Divisi
        const editButton = document.getElementById('editButton');
        const tutupEditModal = document.getElementById('tutupEditModal');
        const editModal = document.getElementById('editModal');

        function passDataEdit($id,$divisi) {
            document.getElementById("form-nama-edit").value=$divisi;
            editModal.classList.remove('hidden');

            tutupEditModal.addEventListener('click', () => {
                editModal.classList.add('hidden');
            });

            editButton.addEventListener('click', () => {
                var url = '{{ route("divisi.edit.submit", ":id") }}';
                url = url.replace(':id', $id);
                document.getElementById("form-edit-divisi").action=url;
            });
        }

        // Modal Hapus Divisi
        const tidakHapuButton = document.getElementById('tidakHapuButton')
        ...
        function passDataDelete($id){
            ...
            tidakHapuButton.addEventListener('click', () => {
                hapusModal.classList.add('hidden');
            });

            hapusButton.addEventListener('click', () => {
                var url = '{{ route("divisi.delete", ":id") }}';
                url = url.replace(':id', $id);
                document.getElementById("form-hapus-divisi").action=url;
            });
        }
