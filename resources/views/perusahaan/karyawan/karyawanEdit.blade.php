                ...
                <div class=" p-10 mx-5 bg-white rounded-3xl drop-shadow-lg shadow-inner lg:w-1/2 ">
                    ...
                    <div class="mb-4 flex items-end justify-end">
                        <a href="{{ Route("karyawan.ganti.password.show",$data->id) }}">
                            <button class="w-auto px-10 font-medium text-sm text-Neutral/01 bg-Neutral/05 hover:bg-orange-300 rounded-md outline-none leading-tight py-3" type="button">Ganti Password</button>
                        </a>
                    </div>
                    ...
                    <form action="{{ route('karyawan.edit.submit',$data->id) }}" method="post">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="nama" name="nama" required value="{{ $data->namaKaryawan }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-4">
                            <label for="tanggal_lahir" class="block font-semibold text-sm text-Neutral/06">*Tanggal Lahir Karyawan</label>
                            <input type="date" id="tanggal_lahir" name="tanggal_lahir" required value="{{ $data->tanggalLahir }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-4">
                            <label for="jenis_kelamin" class="block font-semibold text-sm text-Neutral/06">*Jenis Kelamin Karyawan</label>
                            <select id="jenis_kelamin" name="jenis_kelamin" required value="{{ $data->jenisKelamin }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="alamat" name="alamat" required value="{{ $data->alamat }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-4">
                            <label for="phone" class="block font-semibold text-sm text-Neutral/06">*No.Handphone Karyawan</label>
                            <input type="tel" id="phone" name="phone" required value="{{ $data->noHp }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-4">
                            <label for="divisi" class="block font-semibold text-sm text-Neutral/06">*Divisi Karyawan</label>
                            <select id="divisi" name="divisi" required value="{{ $data->idDivisi }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                                @foreach ($dataId as $no=> $dataId)
                                <option value="{{ $dataId->id }}">{{ $dataId->divisi }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-10">
                            <label class="font-semibold text-sm text-Neutral/06" for="email" for="email">*Email</label>
                            <input type="email" id="email" name="email" placeholder="" value="{{ $data->email }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
