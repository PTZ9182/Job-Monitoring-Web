            ...
            <div class="mx-10 mt-5 lg:mx-20 lg:mt-14 ">
                ...
                <div class="flex flex-col items-center mt-6 lg:flex-row lg:justify-between">
                    <div class=" w-full mt-5 lg:flex lg:items-center lg:justify-center lg:w-1/2 lg:mt-0">
                        <a href="{{ Route("karyawan.karyawan.edit.show") }}" class="bg-Neutral/15  flex w-full  justify-center px-1 py-4 rounded-lg drop-shadow-lg shadow-inner lg:w-1/2">
                            <span class="font-medium text-sm ml-4">Edit Profil</span>
                         </a>
                    </div>
                    
                    <div class="w-full mt-5 lg:flex lg:items-center lg:justify-center lg:w-1/2 lg:mt-0">
                          <a href="{{ Route("karyawan.karyawan.ganti.password.show") }}" class="bg-Neutral/01 hover:bg-Neutral/15  flex w-full justify-center px-1 py-4 rounded-lg drop-shadow-lg shadow-inner lg:w-1/2">
            ...
            <div class="my-10 lg:flex lg:flex-col lg:items-center">
                <div class=" p-10 mx-5 bg-white rounded-3xl drop-shadow-lg shadow-inner lg:w-1/2 ">
                    ...
                    {{-- <a href="#" class="flex w-full items-center justify-center m-3">
                     ...
                     <form action="{{ route('karyawan.karyawan.edit.submit',Auth::guard('karyawan')->user()->id) }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label for="nama" class="block font-semibold text-sm text-Neutral/06">*Nama Karyawan</label>
                            <input type="text" id="nama" name="nama" required value="{{ Auth::guard('karyawan')->user()->namaKaryawan }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-4">
                            <label for="tanggal_lahir" class="block font-semibold text-sm text-Neutral/06">*Tanggal Lahir Karyawan</label>
                            <input type="date" id="tanggal_lahir" name="tanggal_lahir" required value="{{ Auth::guard('karyawan')->user()->tanggalLahir }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-4">
                            <label for="jenis_kelamin" class="block font-semibold text-sm text-Neutral/06">*Jenis Kelamin Karyawan</label>
                            <select id="jenis_kelamin" name="jenis_kelamin" required required value="{{ Auth::guard('karyawan')->user()->jenisKelamin }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="alamat" name="alamat" require required value="{{ Auth::guard('karyawan')->user()->alamat }}"d class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-4">
                            <label for="phone" class="block font-semibold text-sm text-Neutral/06">*No.Handphone Karyawan</label>
                            <input type="tel" id="phone" name="phone" required required value="{{ Auth::guard('karyawan')->user()->noHp }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
                        <div class="mb-10">
                            <label class="font-semibold text-sm text-Neutral/06" for="email" for="email">*Email</label>
                            <input type="email" id="email" name="email" placeholder="" required required value="{{ Auth::guard('karyawan')->user()->email }}" class="w-full font-normal text-sm text-Neutral/08 border border-black rounded-md outline-none leading-tight py-3 px-2">
                        </div>
